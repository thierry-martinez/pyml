let () =
  Pyml_tests_common.add_test ~title:"version" (fun () ->
      Printf.printf "Python version %s\n%!" (Py.version ());
      Pyml_tests_common.Passed)

let () =
  Pyml_tests_common.add_test ~title:"library version" (fun () ->
      Printf.printf "Python library version %s\n%!" (Py.get_version ());
      Pyml_tests_common.Passed)

let () =
  Pyml_tests_common.add_test ~title:"hello world" (fun () ->
      assert (Py.Run.simple_string "print('Hello world!')");
      Pyml_tests_common.Passed)

let () =
  Pyml_tests_common.add_test ~title:"class" (fun () ->
      ...
      let c =
        Py.Class.init "myClass"
          ~methods:[ ("callback", Py.Callable.of_function_as_tuple callback) ]
      in
...
let () =
  Pyml_tests_common.add_test ~title:"empty tuple" (fun () ->
      assert (Py.Tuple.create 0 = Py.Tuple.empty);
      Pyml_tests_common.Passed)

let () =
  Pyml_tests_common.add_test ~title:"make tuple" (fun () ->
      assert (
...
let () =
  Pyml_tests_common.add_test ~title:"module get/set/remove" (fun () ->
...
let () =
  Pyml_tests_common.add_test ~title:"capsule" (fun () ->
...
let () =
  Pyml_tests_common.add_test ~title:"exception" (fun () ->
      try
...
let () =
  Pyml_tests_common.add_test ~title:"ocaml exception" (fun () ->
      let m = Py.Import.add_module "test" in
      let mywrap _ = raise (Py.Err (Py.Err.Exception, "Great")) in
...
let () =
  Pyml_tests_common.add_test ~title:"ocaml other exception" (fun () ->
      let m = Py.Import.add_module "test" in
      ...
      try
        ...
        Pyml_tests_common.Failed "Uncaught exception"
      with Exit -> Pyml_tests_common.Passed)

let () =
  Pyml_tests_common.add_test ~title:"run file with filename" (fun () ->
      let result =
        Pyutils.with_temp_file "print(\"Hello, world!\")" (fun file _channel ->
            Py.Run.load (Py.Filename file) "test.py")
      in
      if result = Py.none then Pyml_tests_common.Passed
      else
...
let () =
  Pyml_tests_common.add_test ~title:"run file with channel"
    (Pyml_tests_common.enable_only_on_unix (fun () ->
         let result =
           Pyutils.with_temp_file "print(\"Hello, world!\")"
             (fun _file channel -> Py.Run.load (Py.Channel channel) "test.py")
         in
         if result = Py.none then Pyml_tests_common.Passed
         else
           let result_str = Py.Object.to_string result in
           let msg =
             Printf.sprintf "Result None expected but got %s" result_str
           in
           Pyml_tests_common.Failed msg))

let () =
  Pyml_tests_common.add_test ~title:"boolean" (fun () ->
      try
...
*)
let () =
  Pyml_tests_common.add_test ~title:"string conversion error" (fun () ->
      try
...
let () =
  Pyml_tests_common.add_test ~title:"float conversion error" (fun () ->
      try
...
let () =
  Pyml_tests_common.add_test ~title:"long conversion error" (fun () ->
      try
...
let () =
  Pyml_tests_common.add_test ~title:"iterators" (fun () ->
...
let () =
  Pyml_tests_common.add_test ~title:"Dict.iter" (fun () ->
...
let () =
  Pyml_tests_common.add_test ~title:"unicode" (fun () ->
...
let () =
  Pyml_tests_common.add_test ~title:"interactive loop"
    (Pyml_tests_common.enable_only_on_unix (fun () ->
...
let () =
  Pyml_tests_common.add_test ~title:"IPython"
...
let () =
  Pyml_tests_common.add_test ~title:"Marshal" (fun () ->
...
let () =
  Pyml_tests_common.add_test ~title:"Py.List.of_list" (fun () ->
...
let () =
  Pyml_tests_common.add_test ~title:"Py.List.sort" (fun () ->
...
let () =
  Pyml_tests_common.add_test ~title:"array" (fun () ->
      let array = [| 1; 2 |] in
...
let () =
  Pyml_tests_common.add_test ~title:"numpy" (fun () ->
...
let () =
  Pyml_tests_common.add_test ~title:"numpy crunch" (fun () ->
      if Py.Import.try_import_module "numpy" = None then
        Pyml_tests_common.Disabled "numpy is not available"
      else
        ...
        let add =
          Py.Module.get_function (Py.Import.import_module "numpy") "add"
        in
        let rec crunch numpy_array n =
          if n <= 0 then numpy_array
          else
            let array =
              Stdcompat.Float.Array.map_from_array Stdcompat.Fun.id
                (Py.Sequence.to_array_map Py.Float.to_float
                   (add [| numpy_array; numpy_array |]))
            in
            crunch (Py.Array.numpy array) (pred n)
        in
...
let () =
  Pyml_tests_common.add_test ~title:"none" (fun () ->
      let none = Py.none in
      assert (none = Py.none);
...
let () =
  Pyml_tests_common.add_test ~title:"docstring" (fun () ->
      Gc.full_major ();
      let fn =
...
let () =
  Pyml_tests_common.add_test ~title:"is-instance" (fun () ->
...
let () =
  Pyml_tests_common.add_test ~title:"is-subclass" (fun () ->
      let int = Py.Dict.find_string (Py.Eval.get_builtins ()) "int" in
...
let () = if not !Sys.interactive then Pyml_tests_common.main ()
